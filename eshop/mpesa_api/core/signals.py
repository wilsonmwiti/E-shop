...
from mpesa_api.core.models import B2CRequest, OnlineCheckout, OnlineCheckoutResponse
...
def handle_response_post_save(sender, instance, **kwargs):
    payment = OnlineCheckout.objects.get(merchant_request_id=(instance.merchant_request_id))
    if instance.phone is not None:
        order = payment.for_order
        print(order)
